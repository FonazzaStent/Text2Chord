"""Text to Chord 1.3.1 - Convert text to chords.
...
def create_textbox():
    ...
    textbox.focus_set()
    textbox.bind("<Button-3>", textbox_context_menu)
...
def chord_display():
    ...
    chord_one.bind("<Button-3>", chord_one_context_menu)
    ...
    chord_two.bind("<Button-3>", chord_two_context_menu)
    ...
    chord_three.bind("<Button-3>", chord_three_context_menu)
...
def create_menu():
    ...
    sub_menu.add_command(compound="left", label="Paste", command=textbox_paste, accelerator="Alt+P")
...
#ContextMenu
def textbox_context_menu(event):
    menu = tk.Menu(root, tearoff = 0)
    menu.add_command(label="Copy", command=textbox_copy)
    menu.add_command(label="Paste",command=textbox_paste)
    try: 
        menu.tk_popup(event.x_root, event.y_root)
    finally: 
        menu.grab_release()
...
def chord_one_context_menu(event):
    menu = tk.Menu(root, tearoff = 0)
    menu.add_command(label="Copy", command=chord_one_copy)
    try: 
        menu.tk_popup(event.x_root, event.y_root)
    finally: 
        menu.grab_release()
...
def chord_one_copy():
    chord_one.event_generate(("<<Copy>>"))

def chord_two_context_menu(event):
    menu = tk.Menu(root, tearoff = 0)
    menu.add_command(label="Copy", command=chord_two_copy)
    try: 
        menu.tk_popup(event.x_root, event.y_root)
    finally: 
        menu.grab_release()
...
def chord_two_copy():
    chord_two.event_generate(("<<Copy>>"))

def chord_three_context_menu(event):
    menu = tk.Menu(root, tearoff = 0)
    menu.add_command(label="Copy", command=chord_three_copy)
    try: 
        menu.tk_popup(event.x_root, event.y_root)
    finally: 
        menu.grab_release()
        
def chord_three_copy():
    chord_three.event_generate(("<<Copy>>"))
...
def aboutbox():
    ...
    url_title.configure(text="Text to Chord 1.3.1", font=("Arial",15))
    ...
    url_label2.configure(text="https://fonazzastent.com/turn-text-into-chords/")
    url_label2.bind("<Button-1>", lambda e: callback("https://fonazzastent.com/turn-text-into-chords/"))
